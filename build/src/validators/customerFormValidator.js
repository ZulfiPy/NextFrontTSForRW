...
const customerSchema = z.object({
    firstName: z.string().min(2, { message: "First name must be at least 2 characters" }).max(50, { message: "First name must fit into 50 characters" }),
    lastName: z.string().min(2, { message: "First name must be at least 2 characters" }).max(50, { message: "First name must fit into 50 characters" }),
    isEstonianResident: z.boolean({ required_error: "Resident field is required" }),
    isikukood: isikukoodSchema,
    day: daySchema,
    month: monthSchema,
    year: yearSchema,
    driversLicenseNumber: z.string().min(4, { message: "Drivers license number must be at least 4 characters" }).max(50, { message: "Drivers license number must fit into 50 characters" }),
    address: z.string().min(2, { message: "Address must be at least 3 characters" }).max(255, { message: "Address must fit into 255 characters" }),
