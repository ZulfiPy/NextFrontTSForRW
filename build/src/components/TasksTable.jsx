...
const TasksTable = () => {
    ...
    useEffect(() => {
        function getTasks() {
            return __awaiter(this, void 0, void 0, function* () {
                if (loading) {
                    try {
                        const response = yield fetch(`${BACKEND_API_DOMAIN}/tasks`, {
                            method: "GET",
                            credentials: "include"
                        });
    ...
    return (<div className={cn("text-left border-2 rounded-lg p-4", { "h-1/3 overflow-y-auto": tasks.length >= 4 })}>
            {loading ?
            ...
            tasks.length > 0 ?
                (<Table>
                        ...
                        <TableBody>
                            {tasks.map((task, index) => (<TableRow key={index}>
                                    ...
                                    <TableCell>
                                        <div className="flex space-x-2">
                                            <div>
                                                <button type="button" onClick={() => router.push(`/components/tasks/${task.id}/view`)}>
                                                    <Eye />
                                                </button>
                                            </div>

                                            <div>
                                                <button type="button" onClick={() => router.push(`/components/tasks/${task.id}/edit`)}>
                                                    <SquarePen />
                                                </button>
                                            </div>
